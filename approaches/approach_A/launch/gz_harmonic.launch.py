...
def generate_launch_description():
    pkg = get_package_share_directory('approach_a')

    urdf_file = os.path.join(pkg, 'urdf', 'mycobot_320_m5_2022_adaptive_gripper.urdf')
    controllers_file = os.path.join(pkg, 'config', 'controllers.yaml')
    world_file = os.path.join(pkg, 'worlds', 'mycobot_world.sdf')
    cube_sdf_file = os.path.join(pkg, 'models', 'cube', 'model.sdf')
    ...
    set_home = ExecuteProcess(
        cmd=[FindExecutable(name='ros2'), 'run', 'approach_a', 'set_home_pose'],
        output='screen'
    )
